...
\begin{table}[t]
  ...
  \input{tables/tab_routing_summary}
\end{table}
...
\begin{table*}[t]
  ...
  \input{tables/tab_routing_plain}
\end{table*}
...
\begin{table*}[t]
  ...
  \caption{Two queries traced through the graph. A: \emph{``Compare Kohli's
    ...
    figure the agent cites.}
  \label{tab:prompt-flow}
  \input{tables/tab_prompt_flow}
\end{table*}

The supervisor router (N10) decides which of the twelve stakeholder agents
answer a query. We compare two routers behind the same interface. The
\emph{keyword router} scores each agent by the words of its keyword list that
appear in the query, plus two structural signals from the analytics (a low
visual confidence and a large strike-rate spread between shots). The
\emph{LLM router} is a locally hosted language model, Qwen2.5-7B served through
Ollama, called with temperature~0 and a fixed seed. To keep the comparison fair,
the model is given exactly what the keyword router knows---each agent's
stakeholder name and keyword list---and its reply is constrained by a JSON
schema to one to four identifiers drawn from the agent registry, so it cannot
name an agent that does not exist. The instructions were fixed before the
comparison was run.

Both routers only \emph{propose}. The same rules are then applied to either
proposal: an agent whose cited facts do not exist for the query is removed, at
most four agents are kept, and if none remain a default pair (Coaches, Players)
answers. Because these rules follow the proposal, the bounds of
Section~\ref{sec:exec-profile} hold whichever router is used.
...
The prompts and their intended targets were written by the authors, so the
targets describe the prompts rather than serve as ground truth, and twelve
plain prompts are too few for a benchmark; the comparison is indicative. The
stability result was measured on one machine; floating-point differences
across GPUs could, rarely, change a decision.
