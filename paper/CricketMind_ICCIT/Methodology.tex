...
% NOTE (edit): Clip.png still needs four fixes before submission --
%  1. N3 must receive input on BOTH paths (every query has text), not only on
%     the "no clip" path; the text below and the implementation both say so.
%  2. The red "Regenerate" loop must return to the agents (only the failed
%     ones are re-run), not to the N10 router.
%  3. The agents inside each column are drawn as a chain (N11a -> N11b -> ...);
%     they run in parallel, so they should hang off one dispatch bus.
%  4. Typos: "States Retrieval", "States Analysis Engine", "States DB" should be
%     "Stats ...", and "failthful" should be "faithful".
\begin{figure*}[t]
  \centering
  \includegraphics[width=0.88\textwidth]{Clip.png}
